...
function detectIntent(q: string): ForecastResponse["intent"] {
  ...
  // What-if / capacity / scenario: students needed, rate change, add students, take time off, replace income
  if (
    ...
    if (s.includes("$") || s.includes("hour") || s.includes("hora") || s.includes("小时") || /\d+k?\b|\d{2,}/.test(q)) return "what_if";
  }
  // Tax
  if (/\btax|irs|1099|quarterly\s+tax|set\s+aside|impuesto|税|预留|退休/i.test(s)) return "tax_estimate";
  // Cash flow / stability / volatility
  ...
  if (
    ...
    /\bstudent\s+pays\s+most|who\s+pays\s+the\s+most|earned\s+me\s+the\s+most|least|estudiante.*más|哪个学生.*最多|谁.*最多/i.test(s) ||
    /\baverage\s+hourly|avg\s+hourly|tarifa\s+promedio|平均.*时薪|时薪/i.test(s) ||
    /\blowest\s+rate|highest\s+rate|below\s+average\s+rate|rate\s+increase|谁.*最低|谁.*最高|低于平均/i.test(s) ||
    /\bcash\s+vs\s+venmo|payment\s+method|método\s+de\s+pago|现金|venmo|zelle/i.test(s) ||
    /\blessons?\s+last\s+month|how\s+many\s+lessons|avg\s+lessons|revenue\s+per\s+lesson|revenue\s+per\s+student|revenue\s+per\s+hour|多少.*课|平均.*课/i.test(s) ||
    /\bbest\s+day\s+of\s+week|day\s+earns?\s+most|哪天.*最多/i.test(s) ||
    /\bcompare\s+to\s+last\s+month|this\s+month\s+vs|growth\s+rate|year[- ]over|同比|环比/i.test(s) ||
    /\brevenue\s+concentration|top\s+3\s+students?|80%\s+of\s+revenue|percent(age)?\s+from\s+top|集中|占比/i.test(s) ||
